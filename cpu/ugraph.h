...
public:
    ...
    int bfs_topdown(std::vector<int> &dist, int source, std::vector<int> &sigma, std::vector<int> &queue,
    ...
    void node_betweenness(std::vector<std::vector<float>> &result, std::vector<int> &queue, 
                          std::vector<int> &dist, std::vector<int> &sigma, int &queue_size);
...
/**
 * Random Graph Generator with given node and edge counts
 * */
...
int UGraph::bfs_topdown(std::vector<int> &dist, int source, std::vector<int> &sigma, std::vector<int> &queue,
                        std::vector<int> &dist_counter, const std::vector<bool> &requested, int &queue_size) 
{
    queue[0] = source;
    dist[source] = 0;
...
void UGraph::compute_centralities(std::vector<std::vector<float>> &result, const std::vector<bool> &requested)
{
    ...
    for(int s = 0; s < num_nodes; s++) { // for all source vertices
        ...
        std::vector<int> sigma(num_nodes, 0);            // number of shortest paths from source to index
        ...
        if(requested[1]) result[1][s] = (float) dist_counter[1] + dist_counter[2];
        // Closeness calculation
        if(requested[2]) result[2][s] = (float) num_nodes * node_closeness(dist_counter, max_dist);
